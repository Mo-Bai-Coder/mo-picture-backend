-- 创建库
create database if not exists person_project;
...
-- 图片表
create table if not exists picture_picture
(
    id           bigint auto_increment comment 'id' primary key,
...
CREATE INDEX idx_reviewStatus ON picture_picture (reviewStatus);
...
-- 空间表
create table if not exists picture_space
(
    id         bigint auto_increment comment 'id' primary key,
...
-- 创建索引
CREATE INDEX idx_spaceId ON picture_picture (spaceId);
...
-- 支持空间类型，添加新列
ALTER TABLE picture_space
    ADD COLUMN spaceType int default 0 not null comment '空间类型：0-私有 1-团队';

CREATE INDEX idx_spaceType ON picture_space (spaceType);

-- 空间成员表
create table if not exists picture_space_user
(
    id         bigint auto_increment comment 'id' primary key,
